...
pub struct Module<I: Ipc, E: Executor> {
    ctx: Option<executor::Context<I, E>>,
    config: Config,
}

impl<I: Ipc, E: Executor> Module<I, E> {
    pub fn new(ctx: executor::Context<I, E>, trait_map: HashMap<String, TraitId>, id: String, args: Vec<u8>) -> Self {
        ...
        let config_remote_trait = RemoteTraitConfig {
        ...
        result.send(&config);
        result.send(&config_remote_trait);
        result
    }
...
impl<I: Ipc, E: Executor> Drop for Module<I, E> {
    fn drop(&mut self) {
        self.send(&"terminate");
...
pub type Modules<I, E> = HashMap<String, Module<I, E>>;

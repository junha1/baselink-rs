...
pub fn generate_imported_struct(
    ...
    let mut imported_struct = quote! {
        ...
        pub struct #struct_ident {
            handle: #env_path::HandleInstance
        }
    };
    ...
    imported_struct.extend(quote! {
        impl #env_path::Service for #struct_ident {
            ...
            fn get_trait_id(&self) -> #env_path::TraitId {
                #trait_id_ident.load(#env_path::ID_ORDERING)
            }
        }
        ...
        impl #env_path::ServiceDispatcher for #struct_ident  {
            fn dispatch(&self, _method: #env_path::MethodId, _arguments: &[u8], _return_buffer: std::io::Cursor<&mut Vec<u8>>) {panic!()}
        }
    });

...
macro_rules! context_provider {
    ($context: ty) => {
        ...
        pub mod context_provider_mod {
            ...
            static POOL: OnceCell<RwLock<HashMap<remote_trait_object::InstanceKey, Box<Context>>>> = OnceCell::new();
            pub fn get() -> &'static Context {
                let ptr: *const _ = &*POOL.get().unwrap().read().get(&remote_trait_object::get_key()).unwrap();
            ...
            pub fn set(ctx: Context) {
                ...
                assert!(!pool.contains_key(&remote_trait_object::get_key()));
                pool.insert(remote_trait_object::get_key(), Box::new(ctx));
            }
            pub fn remove() {
                POOL.get().unwrap().write().remove(&remote_trait_object::get_key()).unwrap();
            }
        }

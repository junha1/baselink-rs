...
fn create_port(
    ...
    config_remote_trait: &RemoteTraitConfig,
) -> Port {
...
pub fn run_control_loop<I: Ipc, H: HandlePreset>(
    ...
    let config_remote_trait: RemoteTraitConfig = recv(&ctx);
    ...
    let ports = RwLock::new(PortTable {
        config_remote_trait: config_remote_trait.clone(),
        map: HashMap::new(),
    });
    global::set(ports);
        ...
        if message == "link" {
            ...
            let old = port_table.map.insert(
                ...
                (
                    counter_module_id,
                    counter_port_id,
                    create_port(port_id, ipc_type, ipc_config, dispather, instance_key, &config_remote_trait),
                ),
            );

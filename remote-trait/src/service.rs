...
macro_rules! service_export {
    ($service_trait: path, $port_id: expr, $arg: expr) => {
        <dyn $service_trait as remote_trait::env::ExportService<dyn $service_trait>>::export($port_id, $arg)
    };
}
...
macro_rules! service_import {
    ($service_trait: path, $arg: expr) => {
        <dyn $service_trait as remote_trait::env::ImportService<dyn $service_trait>>::import($arg)
    };
}
...
macro_rules! service_dispatch {
    ($service_trait: path, $object: expr, $method: expr, $arguments: expr, $return_buffer: expr) => {
        <dyn $service_trait as remote_trait::env::DispatchService<dyn $service_trait>>::dispatch(
            $object,
            $method,
            $arguments,
            $return_buffer,
...
macro_rules! service_id {
    ($service_trait: path) => {
        <dyn $service_trait as remote_trait::env::IdOfService<dyn $service_trait>>::id()
    };
}
